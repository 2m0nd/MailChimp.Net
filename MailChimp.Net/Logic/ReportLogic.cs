...
namespace MailChimp.Net.Logic
{
    ...
    internal class ReportLogic : BaseLogic, IReportLogic
    {
        ...
        /// <exception cref="NotSupportedException"><paramref name="element" /> is not a constructor, method, property, event, type, or field. </exception>
        /// <exception cref="TypeLoadException">A custom attribute type cannot be loaded. </exception>
        public async Task<OpenReportResponse> GetOpenReportAsync(string campaignId, QueryableBaseRequest request = null)
        {
            ...
            using (var client = CreateMailClient("reports/"))
            {
                ...
                var clickReportResponse = await response.Content.ReadAsAsync<OpenReportResponse>().ConfigureAwait(false);
